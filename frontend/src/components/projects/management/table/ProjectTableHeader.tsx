...
interface ProjectTableHeaderProps {
	...
	onFilterClose: () => void;
	onStatusSelect: (status: 'all' | 'active' | 'inactive') => void;
}
		...
		<Box sx={{
			...
			<Box sx={{ display: 'flex', gap: 1.5 }}>
				...
				>
					...
					<MenuItem onClick={() => onStatusSelect('active')} selected={statusFilter === 'active'}>Status: Active</MenuItem>
					<MenuItem onClick={() => onStatusSelect('inactive')} selected={statusFilter === 'inactive'}>Status: Inactive</MenuItem>
				</Menu>
			</Box>
		</Box>
	);
