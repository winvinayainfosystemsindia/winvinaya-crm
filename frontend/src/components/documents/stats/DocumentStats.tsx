...
/**
 * DocumentStats — Comprehensive Document Collection Dashboard.
 * Displays overall document target, PwD breakdown, Non-PwD breakdown,
 * and individual candidate compliance levels.
 */
const DocumentStats: React.FC = () => {
	...
	const totalCandidates = stats.docs_total || (stats.pwd_candidates || 0) + (stats.non_pwd_candidates || 0) || 0;
	const filesCollected = stats.files_collected || 0;
	const filesToCollect = stats.files_to_collect || 0;
	const filesPending = stats.files_pending ?? Math.max(0, filesToCollect - filesCollected);
	...
	const pwdFilesToCollect = stats.pwd_files_to_collect || (pwdCount * 9);
	...
	const nonPwdFilesToCollect = stats.non_pwd_files_to_collect || (nonPwdCount * 8);
	...
	// Consistent paper styling for all cards
	const cardPaperSx = {
		p: 2.2,
		height: '100%',
		display: 'flex',
	...
	return (
		<Box sx={{ mb: 3 }}>
			<Grid container spacing={2.5}>
				{/* Card 1: Files Collected Overview */}
					...
					<Paper elevation={0} sx={{ ...cardPaperSx, borderLeft: `4px solid ${theme.palette.primary.main}` }}>
						<Stack spacing={1.2}>
							<Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
								...
								<Box sx={{ p: 0.8, borderRadius: 1.5, bgcolor: alpha(theme.palette.primary.main, 0.1), color: 'primary.main', display: 'flex' }}>
							...
							<Box>
								<Stack direction="row" spacing={1} alignItems="baseline">
									...
									<Typography variant="caption" color="text.secondary" sx={{ fontWeight: 600 }}>
										/ {filesToCollect} Target Docs
									</Typography>
								</Stack>

								<Stack direction="row" spacing={1} sx={{ mt: 1 }}>
									<Chip
										label={`${filesPending} Pending`}
										size="small"
										color="warning"
										variant="outlined"
					...
					<Paper elevation={0} sx={{ ...cardPaperSx, borderLeft: '4px solid #059669' }}>
						<Stack spacing={1.2}>
							...
							<Typography variant="caption" sx={{ color: 'text.secondary', fontWeight: 600, display: 'block' }}>
								Target: <strong>{pwdFilesToCollect}</strong> docs ({pwdCount} × 9 docs)
							</Typography>
				...
				<Grid size={{ xs: 12, sm: 6, md: 3 }}>
					<Paper elevation={0} sx={{ ...cardPaperSx, borderLeft: '4px solid #6366f1' }}>
						<Stack spacing={1.2}>
							...
							<Typography variant="caption" sx={{ color: 'text.secondary', fontWeight: 600, display: 'block' }}>
								Target: <strong>{nonPwdFilesToCollect}</strong> docs ({nonPwdCount} × 8 docs)
							</Typography>
					...
					<Paper elevation={0} sx={{ ...cardPaperSx, borderLeft: '4px solid #0d9488' }}>
						<Stack spacing={1.2}>
							<Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
								...
								<Box sx={{ p: 0.8, borderRadius: 1.5, bgcolor: alpha('#0d9488', 0.1), color: '#0d9488', display: 'flex' }}>
							...
							<Box>
								...
								<Stack direction="row" spacing={0.8} alignItems="center" flexWrap="wrap" useFlexGap sx={{ mt: 1 }}>
