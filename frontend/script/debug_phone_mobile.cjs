...
const getMaxLength = (countryCode) => {
    ...
    try {
        ...
        if (meta.numberingPlan) {
            // Try mobile first as it's the most common for candidates
            const mobile = meta.numberingPlan.type('mobile');
            const lengths = (mobile && mobile.possibleLengths()) || meta.numberingPlan.possibleLengths();
